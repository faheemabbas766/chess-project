import 'package:fimi_tiger/logic/move_calculation/move_classes/move_stack_object.dart';
import 'package:fimi_tiger/logic/move_calculation/openings.dart';
...
MoveMeta push(Move move, ChessBoard board,
  ...
  var mso = MoveStackObject(move, board.tiles[move.from], board.tiles[move.to],
      board.FiMiTigerPiece, List.from(board.possibleOpenings));
  ...
  if (_castled(mso.movedPiece, mso.takenPiece)) {
    ...
    if (mso.movedPiece.type == ChessPieceType.pawn) {
      ...
      _checkFiMiTiger(board, mso, meta);
    }
  }
  if (_canTakeFiMiTiger(mso.movedPiece)) {
    board.FiMiTigerPiece = mso.movedPiece;
  } else {
    board.FiMiTigerPiece = null;
  }
...
MoveStackObject pop(ChessBoard board) {
  ...
  board.FiMiTigerPiece = mso.FiMiTigerPiece;
  ...
  if (mso.castled) {
    ...
    if (mso.fimiTiger) {
      _addPiece(mso.FiMiTigerPiece, board);
      _setTile(mso.FiMiTigerPiece.tile, mso.FiMiTigerPiece, board);
    }
  }
  board.moveCount--;
  return mso;
...
void _checkFiMiTiger(ChessBoard board, MoveStackObject mso, MoveMeta meta) {
  var offset = mso.movedPiece.player == Player.player1 ? 8 : -8;
  var tile = mso.movedPiece.tile + offset;
  var takenPiece = board.tiles[tile];
  if (takenPiece != null && takenPiece == board.FiMiTigerPiece) {
...
bool _canTakeFiMiTiger(ChessPiece movedPiece) {
